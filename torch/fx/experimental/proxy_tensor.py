...
def proxy_call(proxy_mode, func, args, kwargs):
    ...
    # TODO - I wanted to add this a separate mode, along the lines of
    # FunctinalizeRandomOps. But, when I do
    #   with FunctionaleRandomOps(), proxy_tensor_mode:
    # FunctionalizeRandomOps was getting triggered after proxy_tensor_mode and
    # thus was not traced. For now, I have put it directly in the proxy_call.
    # Another alternative is putting it in decomposition.
    with proxy_mode:
...
class PhiloxRandomState:
    ...
    use_inductor_op = True

    @classmethod
    def reset(cls, tracer=None):
        cls.offset = 0
        cls.seed = torch.initial_seed()

    @classmethod
    def get_offset(cls, shape):
